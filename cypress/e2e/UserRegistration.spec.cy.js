describe('User Registration Form - Create New User', () => {
  ...
  it('Displays a spinner while loading and shows data after loading completes', () => {
    cy.get('login-form-wrap flex w-screen h-screen justify-center').should('be.visible');

    cy.wait('@getUserRegistration');

    cy.get('login-form-wrap flex w-screen h-screen justify-center').should('not.exist');
    // Do I need to verify if the rest of the page has loaded after spinner has completed?
  });
